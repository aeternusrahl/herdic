...
module.exports = new function herdic(){
  ...
  this.getAllBundles = function(){
    var result = [];
    processAllBundlesSync(function(bundle){
      result.push(bundle);
    });
    return result;
  };
  ...
  this.boot = function(){
    ...
    // mark the application as having booted (or at least begun booting)
    hasBooted = true;

    // get a dependency-ordered array of bundle objects
    var allBundles = this.getAllBundles();


    //
    // Configuration phase:
    // Run the config function (if defined) on all bundles
    //
    var bootPromise = allBundles.reduce(function(promise, bundle){
      return promise.then(function(){
        // if there is a config function, invoke it with provider dependencies
        if (bundle.hasOwnProperty('config')) {
          return $q($injector.providerInjector.invoke(bundle.config));
        }
        return null;
      });
    }, $q.when());
    ...
    // Instantiate any providers that haven't already been used yet
    ...
    //
    // Run phase:
    // Execute the Run function on all bundles
    //
    bootPromise = allBundles.reduce(function(promise, bundle){
      return promise.then(function(){
        // if there is a run function, invoke it with instance/service dependencies
        if (bundle.hasOwnProperty('run')) {
          return $q($injector.instanceInjector.invoke(bundle.run));
        }
        return null;
      });
    }, bootPromise);
   ...
   */
  var processAllBundlesSync = function(delegate){
    var DONE = 'done';
    var INPROG = 'inprog';
    var cache = {};
  ...
  var loadComponent = function(component) {
    ...
    if (component.hasOwnProperty('provider')) {
      ...
      $injector.providerInjector.register(component.name + $injector.PROVIDER_SUFFIX, component.provider);
    }
    ...
    // if is service
      ...
      $injector.providerInjector.register(
        ...
        {
          $get: function(){
            return $injector.instanceInjector.instantiate(component.service);
          },
          $injectable: false
        }
      );
